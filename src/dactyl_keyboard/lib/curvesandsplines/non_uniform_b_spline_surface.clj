...
(defn non-uniform-b-spline-surface-point [n p U m q V Pw u v]
    ...
    (doseq [l (range (inc q))]
           ...
           (doseq [k (range (inc p))]
                  (aset temp l (+ (aget temp l)  
                                 (* (nth Nu k) (get-in Pw [(+ u-span (- p) k) (+ v-span (- q) 1)]) ) 
                                     ))
                  )
           )
...
(defn non-uniform-b-spline-surface [n p m q U V Pw steps]
  (for [index-u (range (inc steps))
        :let [u (/ index-u steps)]]
    (for [index-v (range (inc steps))
          :let [v (/ index-v steps)]]
      (non-uniform-b-spline-surface-point n p U m q V Pw u v)))
  )

(comment (non-uniform-b-spline-surface 8 2 4 2 [0 0 0 1 2 3 4 4 5 5 5]
                                       [0 0 0 1 2 3 3 3] 
